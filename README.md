...
知乎直答实测未可靠遵循独立 system message，因此将任务规则、受控候选与显式分隔的用户数据放在单条 user 消息中。服务端仍执行 JSON 校验、候选 ID 白名单、重复 ID 拒绝与行动禁用约束。上游错误、截断或不合规输出返回可重试错误，不伪装成个性化结果。模型 45 秒超时，前端 55 秒；不记录请求正文或上游响应。
