...
assert.equal(quotes.length,1000);assert.equal(new Set(quotes.map(q=>normalize(q.text))).size,1000);
assert.deepEqual(quotes.map(q=>q.id),Array.from({length:1000},(_,i)=>`q${String(i+1).padStart(3,'0')}`));
const upstream=JSON.parse(fs.readFileSync('content/sources/tang-selection.upstream.json','utf8'));
for(const q of quotes.slice(60,600)){const source=upstream.find(p=>p.id===q.upstreamId);assert.ok(source?.paragraphs[q.upstreamParagraph]);assert.ok(q.fullText.includes(q.text.replace('\n','')));assert.ok(q.meaning.length>=20);assert.equal(q.annotationKind,'work-guide');assert.match(q.source,/b8594f81a89752241442f2ce267d6f66f96704ee/)}
for(const q of quotes.slice(600)){assert.equal(q.annotationKind,'source-note');assert.ok(q.meaning.length>=30);assert.ok(['corpus','gutenberg'].includes(q.sourceKind));assert.match(q.source,/^https:\/\//)}
...
console.log('PASS: 1000 unique texts/IDs, source records and guides, stable 24-card legacy pool, bounded 96-candidate retrieval.');
